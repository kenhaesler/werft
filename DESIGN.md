...
**Task inspector.** Open on a plain-language Overview: reported task state, next step, agent provider and attempt, manager attendance, and latest recorded milestone. Separate agent sessions from manager processes. Timeline lists newest milestones first with raw payloads collapsed. Technical identifiers and timestamps live in a disclosure; Evidence and Attempts stay dedicated views. Body text is at least 14px and actions at least 44px tall. Never infer commands or edits from manager attendance; stale telemetry cannot claim live monitoring.
