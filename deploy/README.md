...
| `dns-guard` | Werft-built dnsmasq image (`dns-guard/Dockerfile`). Resolves only the names in `dns-allow.conf`; everything else is `NXDOMAIN`. |
...
file mode the manager's non-root user can't open), or empty/whitespace-only
...
- **`/opt/werft`** — operator-authored config and secrets, mounted read-only.
  ...
  - `config/dns-allow.conf` — generated by `gen-dns-allow.sh` (below),
    bind-mounted into `dns-guard` at `/etc/dnsmasq.d/dns-allow.conf`.
    **This file must exist before the first `docker compose up`** —
    `install.sh` creates it (empty is fine), but if it's missing when
    compose starts, Docker silently creates a *directory* at that path
    instead of bind-mounting a file, and `dns-guard` then fails confusingly
    (dnsmasq can't load a directory as its config).
  - `secrets/pg_password`, `secrets/api_token`, `secrets/github_app_key`,
    `secrets/claude_credential`, `secrets/ntfy_token` — the files
    `compose.yaml`'s top-level `secrets:` block points at.
...
1. Edit `/opt/werft/config/dispatch.json` — add/remove a project's
   ...
   `extra_hosts` entry.
2. Regenerate `dns-allow.conf` from the updated config:
   ```sh
   dns-guard/gen-dns-allow.sh /opt/werft/config/dispatch.json /opt/werft/config/dns-allow.conf
   ```
3. Reload dns-guard without restarting it (SIGHUP re-reads
   `/etc/dnsmasq.d/dns-allow.conf`):
   ```sh
   docker kill -s HUP werft-dns-guard
   ```
4. squid's own per-slot/manager `dstdomain` files under
   `/srv/werft/egress/allow/` are rewritten by the manager itself at claim
   time (not by this script) — no manual squid reload is part of this
   rollout; squid re-reads its `dstdomain` files on each ACL check, no HUP
   needed.
