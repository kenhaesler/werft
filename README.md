...
3. **Providers are subscription CLIs, dispatched at the process layer.** Claude, Codex, Kimi, and Ollama-backed CLIs run headless inside ephemeral runner containers behind thin adapters (~100 lines each: start task, stream log, exit code). The manager meters plan quotas (rolling windows, weekly caps); Ollama is the free local overflow. No gateway, no per-token billing in the core path.
