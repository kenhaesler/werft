...
| Containers | Docker CE + Compose plugin, rootful | **≥ 26.x** (past CVE-2024-29018; current stable) | Rootless/Podman/`userns-remap`: UID-remap friction without proportionate gain — the VM is the declared blast-radius boundary (operator decision 2026-07-25, §12) |
...
- **gVisor / Kata / microVM per-runner isolation** — a multi-tenant threat model Werft doesn't have; gVisor conflicts with SELinux-enforcing (needs `label=disable`), Kata needs nested virt a guest VM can't provide; both add a second runtime path (fails §14). *Revisit:* Werft becomes multi-tenant, or a runc-escape CVE lands with no cheaper mitigation.
- **Docker `userns-remap`** — rejected by operator decision (2026-07-25): the subuid-offset bind-mount ownership complexity buys little when the VM is the declared blast-radius boundary, runners already run as non-root uid 10000, and SELinux enforcing + `CapDrop=ALL` + `no-new-privileges` + read-only rootfs carry the container-hardening load (§6.3). *Revisit:* a runc-escape CVE lands with no cheaper mitigation.
...
15. **Postgres major upgrades go via dump/restore only** (never an in-place `:18`→`:19` tag bump against an existing data volume, which hard-fails on a directory-version mismatch); the image is digest-pinned, and the existing pg_dump backups make the dump/restore path smooth.
...
- **Ledger & risks**: seven new §12 rejects (with revisit triggers) and eight new §13 accepted risks (§13#9–16 as of v1.3; #15 removed and #16 renumbered to #15 in v1.4).

**v1.4 (2026-07-25, operator decision):** `userns-remap` rejected — see the §12 reject ledger entry; the §2 containers row and §13 no longer carry it as a CONSIDER/deferred prototype.
